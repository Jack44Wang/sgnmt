...
class linearModel(Model):
    ...
    def add_placeholders(self):
        ...
        """
        self.input_placeholder = tf.placeholder(tf.float32, [None, self.d_model])
        self.targets_placeholder = tf.placeholder(tf.float32,
                                        [None, self.max_length, self.d_model])
        self.target_mask_placeholder = tf.placeholder(tf.bool,
                                        [None, self.max_length])
        self.dropout_placeholder = tf.placeholder(tf.float32)
        self.reward_holder = tf.placeholder(tf.float32, [None, self.max_length])
        self.probs_holder = tf.placeholder(tf.float32, [None, self.max_length])
    ...
    def create_feed_dict(self, inputs_batch, mask_batch=None,
        ...
        feed_dict = {
            ...
            self.target_mask_placeholder: mask_batch,
    ...
    def add_loss_op(self):
        ...
        """
        mask = reward_holder == 0
        raw_loss = tf.log(self.probs_holder)*self.reward_holder
    ...
    def predict_one_step(self, sess, inputs_batch):
        ...
        Returns:
            ...
            actions: Chosen actions, [batch_size * 1]
                     ...
                     1 -> WRITE
        """
        feed = self.create_feed_dict(inputs_batch, dropout=Config.dropout)
        predictions = sess.run(self.pred, feed_dict=feed)
        probs = sess.run(tf.reduce_max(predictions, axis=1))
        actions = sess.run(tf.max(predictions, axis=1))
